...
from .max_tree_c_01 import (canonicalize_c, computeNodeArray2d_c,
                            computeNodeArray3d_c, counting_sort_c,
                            union_find2d_c, union_find3d_c)
...
def build_max_tree(f, Bc, option=0):
    ...
    # auxiliary array to store level roots
    ...
    if option == 0:
        ...
        if ndim == 2:  # node array/node index representation
            node_array = computeNodeArray2d_c(
                parent, flat_img, S_rev, node_index)
        else:
            node_array = computeNodeArray3d_c(
                parent, flat_img, S_rev, node_index)
