...
try:
    ...
    warnings.warn(
        "gvgen not installed, this may cause a problem in drawing functions")
...
def compact(self, to_remove, lut):
    ...
    new_node_array = np.empty(
        (self.node_array.shape[0], nodes_kept.size), dtype=np.int32)
    self.remove_node_array_lines_aux(
        nodes_kept, new_node_array, self.node_array)
...
def prune(self, to_prune):
    ...
    self.prune_aux(lut, to_prune.astype(np.int32),
                   self.node_array[0, :], self.node_array[1, :])
...
def getChildren(self, node=0):
    ...
    if not self._children_updated:
        self._cum_children_hist = self.node_array[1, :].cumsum().astype(
            np.int32)
        par = self.node_array[0, :]
        self._children_list = self.get_children_aux(
            self._cum_children_hist.copy(), par)
...
def generateCCGraph(self, s=(100, 100), parent_scale=True, LR=False, file_name="graph"):
    ...
    for i in range(1, n_nodes):
        ...
        node_image = (self.recConnectedComponent(
            i)[xpmin:xpmax+1, ypmin:ypmax+1]).astype('uint8')*255
...
def generateCCPathGraph(self, start, end=0, s=(100, 100), parent_scale=True,
    ...
    while True:
        ...
        if parent_scale and pos != end:
            xpmin, xpmax = self.node_array[6, parents[pos]
                                           ], self.node_array[7, parents[pos]]
            ypmin, ypmax = self.node_array[9, parents[pos]
                                           ], self.node_array[10, parents[pos]]
        else:
            xpmin, xpmax = self.node_array[6, pos], self.node_array[7, pos]
            ypmin, ypmax = self.node_array[9, pos], self.node_array[10, pos]

        node_image = (self.recConnectedComponent(
            pos)[xpmin:xpmax+1, ypmin:ypmax+1]).astype('uint8')*255
        node_image = cv2.resize(node_image, (s[1], s[0]))
...
def recConnectedComponent(self, node, bbonly=False):
    ...
    if self.node_index.ndim == 2:
        self.rec_connected_component_2d_aux(
            int(node), int(seed), self.node_index, cc, self.off)
    else:
        self.rec_connected_component_3d_aux(
            int(node), int(seed), self.node_index, cc, self.off)
...
def computeNodeGrayAvg(self):
    ...
    self.compute_node_gray_avg_aux(
        self.node_array[0, :], self.node_array[2, :], self.node_array[3, :].copy(), gray_avg)
...
def computeNodeGrayVar(self, gray_avg=[]):
    ...
    self.compute_node_gray_var_aux(
        self.node_array[0, :], self.node_array[2, :], self.node_array[3, :].copy(), squared_gray_avg)
