...
def computeStabilityMeasure(self, delta=5):
    ...
    self.compute_stability_measure_aux(
        parent, h, area, nlevels, nodes_list, stability_measure, delta, hmin)
...
def draw1DImageTree(self, tree='mt'):
    ...
    for i in xrange(self._cum_sb_hist.size-1, 0, -1):
        ...
        for j in sb:
            ...
            LC = self.recConnectedComponent(
                j).flatten()  # Recovered level component
    ...
    new_nodes_image = np.zeros(
        (nglevels*pixels_size, npixels*pixels_size), dtype='uint8')
    new_nodes_image[pixels_size/2::pixels_size,
                    pixels_size/2::pixels_size] = nodes_image
    ...
    if tree == 'mt':
        ...
        for ii in composite_nodes[1:]:
            x, y = pixels_size/2+pixels_size * \
                ii[1], pixels_size/2+pixels_size*ii[0]
    ...
    for jj in tree_connections:
        pt1 = (pixels_size/2+pixels_size *
               jj[1], pixels_size/2+pixels_size*jj[0])
        pt2 = (pixels_size/2+pixels_size *
               jj[3], pixels_size/2+pixels_size*jj[2])
...
def getSignature(self, attrib, start, end=0, cte=0):
    ...
    signature = self.get_signature_aux(
        par, h, area, attrib, int(start), int(end), int(cte))
...
def hmax(self, h, Height=[]):
    ...
    self.node_array[2, child_height <
                    h] -= (h - child_height[child_height < h])
    return self
...
def vmax(self, vol, V=None):
    ...
    self.node_array[2, child_volume < vol] -= (
        vol - child_volume[child_volume < vol]) / area[child_volume < vol]
    return self
